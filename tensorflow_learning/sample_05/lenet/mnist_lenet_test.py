...
with tf.Session() as sess:
    saver_op.restore(sess, './ckpts/mnist_lenet_result.ckpt')  # 从文件中读取变量
    #测试也用minibatch
    for i in range(100):
        x_batch, y_batch = mnist.test.next_batch(1000)
        #test_accuracy = compute_accuracy(x_batch, y_batch)
        test_accuracy = sess.run(accuracy, feed_dict={xs: x_batch, ys: y_batch, keep_prob: 1})
        sess.run(update, feed_dict={batch_accuracy: test_accuracy})
        print('第%d次测试结果：%g' % (i, sess.run(test_accuracy_sum)))
    print('最后测试结果：%g' % (sess.run(test_accuracy_sum)/100))